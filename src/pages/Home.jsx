...
import { useDebounce } from "use-debounce"; // Import the use-debounce hook
...
function Home() {
  ...
  const fetchData = async () => {
    try {
      ...
      //  saveDatailedRecipesToFirebase(data);
      if (data.length === 0) {
        return;
      }
  ...
  const fetchFilteredData = async () => {
    ...
    try {
      ...
      const recipes = querySnapshot.docs
        ...
        .filter((recipe) => recipe.title.toLowerCase().includes(debouncedSearchTerm.toLowerCase()));
      setData(recipes);
      setLoading(false);
      setError(null);
    } catch (error) {
  ...
  return (
    <div className={style.container}>
      ...
      {/* <IngredientSearch data={data} setData={setData}/> */}
